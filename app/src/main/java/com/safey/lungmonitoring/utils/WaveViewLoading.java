...
public class WaveViewLoading extends View {
    /**
     * +------------------------+
    ...
    private static final float DEFAULT_AMPLITUDE_VALUE = 10.0f;
    private static final float DEFAULT_WATER_LEVEL_RATIO = 0.5f;
    private static final float DEFAULT_WAVE_LENGTH_RATIO = 1.0f;
    private static final float DEFAULT_WAVE_SHIFT_RATIO = 0.0f;
    private static final int DEFAULT_WAVE_PROGRESS_VALUE = 5;
    ...
    private static final int DEFAULT_WAVE_SHAPE = ShapeType.ROUNDRECT.ordinal();
    ...
    private static final float DEFAULT_TITLE_BOTTOM_SIZE = 14.0f;
    ...
    public WaveViewLoading(final Context context) {
        this(context, null);
    }

    public WaveViewLoading(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public WaveViewLoading(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        Typeface plain = Typeface.createFromAsset(getContext().getAssets(), "fonts/Circular_Std_Medium.ttf");
        mCenterTitlePaint.setTypeface(plain);
        ...
        mCenterTitlePaint.setTextAlign(Paint.Align.CENTER);
        ...
        mBottomTitlePaint.setTextAlign(Paint.Align.CENTER);
        mBottomTitlePaint.setTypeface(plain);
        mBottomTitlePaint.setAlpha(60);
        ...
        if (mWaveShader != null) {
            ...
            switch (mShapeType) {
                ...
                case 4:
                        ...
                        if (borderWidth > 0) {
                            RectF rect = new RectF(0, 0, getWidth() - borderWidth / 2f - 0.5f, getHeight() - borderWidth / 2f );
                            canvas.drawRoundRect(rect, mRoundRectangleXY, mRoundRectangleXY, mBorderPaint);
                            canvas.drawRoundRect(rect, mRoundRectangleXY, mRoundRectangleXY, mWaveBgPaint);
                            canvas.drawRoundRect(rect, mRoundRectangleXY, mRoundRectangleXY, mWavePaint);
                        } else {
                            //RectF rect = new RectF(0, 0, getWidth()/2, getHeight()/2);
                            /*canvas.drawRoundRect(rect, 25, 400, mWaveBgPaint);
                            canvas.drawRoundRect(rect, 25, 400, mWavePaint);*/
                            /*Point screenSize = new Point(getWidth(), getHeight());
                            int radius1 = Math.min(screenSize.x, screenSize.y);
                            int cx = (screenSize.x - radius1) / 2;
                            int cy = (screenSize.y - radius1) / 2;


                            //float radius1 = Math.min(p.x, p.y);
                            Path path = new Path();
                            RectF rect = new RectF(0, 0, (radius1 + cx), radius1 + cy  );
                            path.addRoundRect(rect,cx,radius1, Path.Direction.CCW);
                            path.close();
                            canvas.drawPath(path,mWaveBgPaint);
                            canvas.drawPath(path,mWavePaint);*/


                           /* RectF rect = new RectF(0, 0, getWidth() - borderWidth / 2f - 0.5f, getHeight() - borderWidth / 2f - 0.5f);
                            canvas.drawRoundRect(rect, getWidth()-200, getWidth()/2, mBorderPaint);
                            canvas.drawRoundRect(rect, getWidth()-200, getWidth()/2, mWaveBgPaint);
                            canvas.drawRoundRect(rect, getWidth()-200, getWidth()/2, mWavePaint);
*/

                            Path path = new Path();
                            Rect drawingRect = new Rect(0,0,getWidth(),getHeight());
                            ...
                            int topLeftRadius=0;
                            int bottomLeftRadius=getWidth()/2;
                            topRightArcBound.set(0, 0, drawingRect.right, drawingRect.top + topRightRadius * 2);
                            int bottomRightRadius=getWidth()/2;
