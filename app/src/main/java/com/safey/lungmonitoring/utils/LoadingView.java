/*
package com.safey.lungapp.utils;
...
public class LoadingView extends View {
    ...
    public enum Shape {
        CIRCLE(1), SQUARE(2), HEART(3), STAR(4),RoundRect(5);
        int value;
    ...
    public LoadingView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mShape = Shape.fromValue(attributes.getInt(R.styleable.YPWaveView_shapeType, 5));
    ...
    private void resetShapes() {
        ...
        switch (mShape) {
            ...
            case RoundRect:
                ...
                mPathBorder = drawRoundRect(cx, cy, radius);
                mPathContent = drawRoundRect(cx + ((int) mShapePadding / 2), cy + ((int) mShapePadding / 2), radius - (int) mShapePadding);
                break;
            case SQUARE:
    ...
    private Path drawRoundRect(int cx, int cy, int radius) {

  Path path = new Path();
        int offset = 50;
        path.moveTo(cx, cy);
        path.lineTo(cx - offset, offset);
        path.lineTo(cx - offset, cy - offset);
        path.lineTo(offset, cy - offset);
